...
RealAdaBoostClass_CV <- function(lCVData = NA, lArgs = NA, vM= seq(1,15), sName ='', bPlot = TRUE){
  "
    Purpose:
      Perform Cross Validation on vLambda grid. 
      
    Inputs:
      mX            matrix, containing observations training set predictor variables
      vY            vector, containing test observations training set dependent variable
      lIdx          list, indices CV test set per fold. Note: CV splits the input training set in each fold into
                    a CV training set and a CV test set
      fFunc         function, used to estimate parameters 
      lArgs         named list, containing all other function arguments than mX, vY and dLambda
      vLambda       vector, number of weak learners grid 
      sName         string, characteristic part file name
      
    Output value:
      RMSE plot
      
    Return value:
      dLambdaCV    Optimal lambda based on one standard deviation rule, that is, we take the least complex model 
                   [largest lambda] within one standard deviation of the best model. See `The Elements of Statistical 
                   Learning by Hastie, Tibshirani, and Friedman (2008)', page 62 for a graphical explanation 
                   of this rule [note that the largest lambda corresponds to the smallest s in this figure]. 
    "
  # Initialisation
  ...
  Error <- function(iM){
    ...
    for(i in 1: iFold){
      ...
      vError[i] <- RealAdaBoost(mDataTrain = lCVData[[i]]$mDataTrain, mDataTest = lCVData[[i]]$mDataTest, iM = iM, iDepth = iDepth, iSeed = iSeed, bVerbose=FALSE)$Error
    }
